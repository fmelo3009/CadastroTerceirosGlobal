...
| `GET /interno` | Painel: lista de terceirizados com filtros (empreendimento, serviço, cidade, situação) |
| `GET /interno/terceirizados/:id` | Detalhes de um terceirizado (dados, experiência, documentos) |
| `POST /interno/terceirizados/:id/situacao` | Atualiza a situação cadastral (Pendente/Ativo/Inativo/Bloqueado) |
| `GET /interno/documentos/:id/download` | Download de um documento anexado pelo terceirizado |
...
- **Painel interno funcional**: lista de terceirizados com filtros (empreendimento, serviço,
  cidade, situação), tela de detalhes com dados completos + experiência, download dos documentos
  anexados e alteração da situação cadastral direto pelo painel.
...
- Painel interno ainda não tem paginação (ok para o volume atual, mas vale revisar se o número de
  cadastros crescer muito) nem edição de dados do terceirizado (só leitura + mudança de situação).
